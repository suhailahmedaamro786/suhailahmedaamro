...
export default async function CaseStudy({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen">
      ...
      {project.portals && <section className="section-padding bg-gradient-to-br from-primary-50 to-blue-50 dark:from-gray-900 dark:to-gray-800"><div className="container-custom max-w-5xl"><h2 className="text-3xl font-bold mb-2">Live Portals</h2><p className="text-gray-600 dark:text-gray-300 mb-6">Explore each production-facing portal of the NPSD School ERP.</p><div className="grid md:grid-cols-3 gap-5">{project.portals.map((item) => <a key={item.label} href={item.url} target="_blank" rel="noopener noreferrer" className="group p-5 rounded-2xl bg-white dark:bg-gray-800 border dark:border-gray-700 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all"><div className="flex items-center justify-between gap-3"><span className="font-bold">{item.label}</span><ExternalLink size={18} className="text-primary-600"/></div><span className="mt-2 block text-sm text-gray-500 dark:text-gray-400 break-all">{item.url}</span></a>)}</div></div></section>}
